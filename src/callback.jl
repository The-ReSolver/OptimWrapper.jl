...
struct Callback{X, C, OPTIMIZER, S, CB, R, F, RM}
    ...
    function Callback(optimisationVariable, cache, options::OptOptions{OPTIMIZER, S, CB, R, F, RM}) where {OPTIMIZER, S, CB, R, F, RM}
        ...
        new{typeof(optimisationVariable), typeof(cache), OPTIMIZER, S, CB, R, F, RM}(optimisationVariable, cache, options, ifKeepZero, startTime)
    end
end
...
function (f::Callback)(state)
    ...
    ifUpdateTrace(state.iteration, f.ifKeepZero) ? push!(f.options.trace, state, f.startTime) : nothing
