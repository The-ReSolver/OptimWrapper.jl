@testset "Generic Optimisation State    " begin
    ...
    @test convert(OptimWrapper.GenericOptimisationState, dummyState, 5, 0.0) == OptimWrapper.GenericOptimisationState(5 + iter, value, time)
end
...
@testset "First Order Optimisation State" begin
    ...
    @test convert(OptimWrapper.FirstOrderOptimisationState, dummyState, 5, 0.0) == OptimWrapper.FirstOrderOptimisationState(5 + iter, value, gnorm, time, stepSize)
end
...
@testset "Nelder-Mead Optimisation State" begin
    ...
    @test convert(OptimWrapper.NelderMeadOptimisationState, dummyState, 5, 0.0) == OptimWrapper.NelderMeadOptimisationState(5 + iter, value, gnorm, time, stepType)
end
